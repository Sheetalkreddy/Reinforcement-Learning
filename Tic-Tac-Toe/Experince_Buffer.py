...
class Experience_Replay_Buffer():
    ...
    def buffer_get_batch(self,model,batch_size,nb_actions = 9,gamma = 0.99):
        ...
        for i in pop:
            ...
            state_t = self.buff[i][0]

            state_tp1,done = self.buff[i][3]
            reward = self.buff[i][2]
            action = self.buff[i][1]
    ...
    def buffer_get_mmbatch(self,model,batch_size,nb_actions = 9,gamma = 0.99):
        size = min(batch_size,len(self.buff))
        ...
        for i in pop:
            ...
            if done == True:
                ...
                if np.count_nonzero(state_t)%2==0:
                    #Going First

                    targets[count,action] = reward - (gamma)*np.max(Q_sa[0])
                else:
                    targets[count,action] = (-1*reward) - (gamma)*np.min(Q_sa[0])
            count+=1
