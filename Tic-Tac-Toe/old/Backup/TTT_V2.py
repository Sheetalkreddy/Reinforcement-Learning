...
class TTT:
	...
	def act(self,move,ai_mode = 1):
		...
		if self.user_chance == 2:
			if ai_mode == 1:
				ret_1 = self.random_ai(self.ai_user)
			elif ai_mode == 2:
				ret_1 = self.safe_ai()
			if self.done == True:
				return ret_1
...
epc = 1
epi_1 = 1.0# Exploration vs Exploitation
epi_2 = 1.0
...
for ep in range(epc):
	...
	if ep%5000 == 0 and ep != 0 :
		epi_1 = epi_1 - epi_decay
		if ai_mode == 1:
			ai_mode = 2
		else:
			ai_mode = 1
	if ep%10000 == 0 and ep != 0 :
		epi_2 = epi_2 - epi_decay
		if ai_mode == 1:
			ai_mode = 2
		else:
			ai_mode = 1

	if ep%3000 == 0 and ep!=0 :
		env.change_user_chance()
...
for i in range(test_cases):
	...
	while game_over == False:
		...
		input_tm1 = input_t.reshape(1,9)
		q = model.predict(input_tm1,batch_size = 1)

		action = np.argmax(q[0])

		state_tp1,reward = env.act(action,2)

		input_t1 = state_tp1[0]
		game_over = state_tp1[1]
